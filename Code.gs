...
const campaign = decodeURIComponent(campaignRaw);
  ...
  const statusRule = SpreadsheetApp.newDataValidation()
    .requireValueInList(["sent email", "replied", "applied", "accepted", "onboarded"], true)
...
const RECEIVED_AT_COLUMN = 1; // A
const TEAM_MEMBER_COLUMN = 2; // B
const COMPANY_COLUMN = 5; // E
const TITLE_COLUMN = 6; // F
const LINKEDIN_COLUMN = 7; // G
const REFERRAL_LOGGED_AT_COLUMN = 13; // M

function handleAcceptedReferralEdit(e) {
  const sheet = e.range.getSheet();
  ...
  const newStatus = String(e.value || "").toLowerCase().trim();
  if (newStatus !== ACCEPTED_STATUS) return;

  const row = e.range.getRow();
  const referralLoggedAtCell = sheet.getRange(row, REFERRAL_LOGGED_AT_COLUMN);

  if (referralLoggedAtCell.getValue()) return;

  const teamMember = String(sheet.getRange(row, TEAM_MEMBER_COLUMN).getValue() || "").trim();
  const leadName = String(sheet.getRange(row, LEAD_NAME_COLUMN).getValue() || "").trim();

  if (!teamMember || !leadName) {
    sheet.getRange(row, REFERRAL_LOGGED_AT_COLUMN).setValue("Missing team member or lead name");
    return;
  }

  const referralSheet = getOrCreateReferralSheet_(teamMember);

      referralSheet.appendRow([
    leadName,
    getDomainFromCampaign_(sheet.getRange(row, CAMPAIGN_COLUMN).getValue()),
    sheet.getRange(row, EMAIL_COLUMN).getValue(),
    "Accepted"
  ]);

  applyReferralStatusDropdown_(referralSheet.getRange(referralSheet.getLastRow(), 4));

  referralLoggedAtCell.setValue(new Date());
}

function getOrCreateReferralSheet_(teamMember) {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const sheetName = sanitizeSheetName_(teamMember);
...
function getDomainFromCampaign_(campaign) {
  const value = String(campaign || "").toLowerCase();

  if (value.includes("accounting")) return "Accounting";
  if (value.includes("corpfin")) return "Corporate Finance";
  if (value.includes("insurance")) return "Insurance";

  return "";
}

function applyReferralStatusDropdown_(range) {
  const statusRule = SpreadsheetApp.newDataValidation()
    .requireValueInList(["Accepted", "Submitted", "Screening", "Onboarded"], true)
    .setAllowInvalid(false)
    .build();
